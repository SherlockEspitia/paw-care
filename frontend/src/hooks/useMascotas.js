...
import * as service from '@/services/mascotasService';

export default function useMascotas({ initialPage = 1, initialPerPage = 10, initialSearch = '' } = {}) {
  ...
  const fetchMascotas = useCallback(async (opts = {}) => {
    setLoading(true);
    setError(null);
    const params = {
      page: opts.page ?? page,
    ...
    try {
      const resp = await service.listMascotas(params);
  ...
  const refresh = useCallback(() => fetchMascotas({ page, perPage, search }), [fetchMascotas, page, perPage, search]);

  const getMascotaById = useCallback(async (id) => {
    setLoading(true);
    try {
      const resp = await service.getMascota(id);
      return resp;
    } catch (err) {
      setError(err);
      throw err;
  ...
  const create = useCallback(async (payload) => {
    setLoading(true);
    try {
      const created = await service.createMascota(payload);
      await refresh();
      return created;
    } catch (err) {
      setError(err);
  ...
  const update = useCallback(async (id, payload) => {
    setLoading(true);
    try {
      const updated = await service.updateMascota(id, payload);
      await refresh();
      return updated;
    } catch (err) {
      setError(err);
